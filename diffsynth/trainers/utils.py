...
def wan_parser():
    ...
    parser.add_argument("--batch_size", type=int, default=1, help="Per-device batch size used by the DataLoader.")
    ...
    parser.add_argument("--source_concat_mode", type=str, default="token", choices=["token", "channel"], help="Source video concatenation mode: 'token' for sequence concatenation, 'channel' for channel concatenation.")
    parser.add_argument("--channel_concat_patch_init", type=str, default="copy", choices=["copy", "zeros", "random"], help="Patch embedding initialization method when using channel-wise source concat.")
    parser.add_argument("--use_source_attention_mask", action="store_true", help="Enable source-token-only self-attention mask during training/inference.")
    ...
    parser.add_argument("--wandb_project", type=str, default="wan2_1_train") #加wandb
    parser.add_argument("--wandb_run_name", type=str, default=None)
    parser.add_argument(
        "--lr_warmup_steps",
        type=int,
        default=-1,
        help="Number of warmup steps for LR scheduler. If <0, use 10% of total steps.",
    )#加warm-up
    parser.add_argument(
        "--train_mode",
        type=str,
        ...
        help="mix: image+video interleave; image: image only; video: video only",
    )#加训练模式
    parser.add_argument("--bs_img", type=int, default=16, help="Image batch size.")
    parser.add_argument("--bs_vid", type=int, default=4, help="Video batch size.")
    parser.add_argument("--use_siglip_semantic", action="store_true", help="Use SigLIP first-frame semantic tokens during training/inference.")
    parser.add_argument("--siglip_model_path", type=str, default=None, help="Path or HF id for SigLIP model used to extract semantic tokens.")
    parser.add_argument("--semantic_mlp_dim", type=int, default=1024, help="Hidden dim of the SigLIP semantic projection MLP.")
    # EMA
    parser.add_argument("--use_ema", default=False, action="store_true", help="Whether to use EMA.")
    parser.add_argument("--ema_decay", type=float, default=0.999, help="EMA decay.")
    parser.add_argument("--ema_iter", type=int, default=10, help="EMA iteration.")
    # pretrain state dict path
    parser.add_argument("--state_dict_path", type=str, default=None, help="Path to the pretrain state dict.")
    # image list
    parser.add_argument("--image_list", type=str, default=None, help="Path to the image list.")
    # loss skip threshold
    parser.add_argument("--loss_skip_threshold", type=float, default=None, help="Skip training step if loss exceeds this threshold. None means no skipping.")
    parser.add_argument("--loss_skip_step", type=int, default=100, help="The number of iterations before skipping.")
    parser.add_argument("--grad_norm_skip_threshold", type=float, default=None, help="Skip training step if loss exceeds this threshold. None means no skipping.")
    parser.add_argument("--grad_norm_skip_step", type=int, default=100, help="The number of iterations before skipping.")
