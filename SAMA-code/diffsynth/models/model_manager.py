...
def load_model_from_single_file(state_dict, model_names, model_classes, model_resource, torch_dtype, device):
    ...
    for model_name, model_class in zip(model_names, model_classes):
        ...
        if hasattr(model, "eval"):
            model = model.eval()
        model.load_state_dict(model_state_dict, strict=False,  assign=True) #####改成false
        # ==== 修复开始====
        if hasattr(model, "segment_embedding"):
            seg = model.segment_embedding
            if getattr(seg.weight, "is_meta", False):
                num_embeddings, dim = seg.weight.shape
                seg_new = torch.nn.Embedding(num_embeddings, dim)
                # 关键：零初始化，保证一开始不改变原有行为
                torch.nn.init.zeros_(seg_new.weight)
                model.segment_embedding = seg_new

        if hasattr(model, "task_embedding"):
            task = model.task_embedding
            if getattr(task.weight, "is_meta", False):
                num_embeddings, dim = task.weight.shape
                task_new = torch.nn.Embedding(num_embeddings, dim)
                # 关键：零初始化，保证一开始不改变原有行为
                torch.nn.init.zeros_(task_new.weight)
                model.task_embedding = task_new
        # ==== 修复结束 ====
