...
class WanVideoPipeline(BasePipeline):
    ...
    def training_loss(self, **inputs):
        ...
        # forward_begin = time.time()
        noise_pred = self.model_fn(**inputs, timestep=timestep, source_concat_mode=self.source_concat_mode)
        
        # if timing_enabled and forward_begin is not None:
        #     if torch.cuda.is_available():
        #         torch.cuda.synchronize()
        ...
        # forward_time = time.time() - forward_begin
        # print("loss_time is {:.2f}s".format(forward_time))
        # if timing_enabled:
        #     preprocess_time = self._wan_debug_consume_time("preprocess")
        #     vae_time = self._wan_debug_consume_time("vae")
        #     dataload_time = max(preprocess_time - vae_time, 0.0)
        #     total_time = dataload_time + vae_time + forward_time
        #     self._wan_debug_log(
        #         f"[WanDebug] stage=train_loss, dataload={dataload_time:.2f}s, vae={vae_time:.2f}s, "
        #         f"forward={forward_time:.2f}s, total={total_time:.2f}s"
        #     )
        return loss
    ...
    def _adjust_dit_input_dim_for_channel_concat(self, init_mode: Optional[str] = None):
        ...
        def adjust_single_dit(dit_model):
            ...
            # Preserve original dtype/device for the new layer
            ...
            # Create new patch embedding layer with doubled input channels
            ...
            with torch.no_grad():
                # Always preserve pretrained weights for the original (target) channels
                orig_weight = dit_model.patch_embedding.weight.to(orig_dtype)
                new_patch_embedding.weight[:, :current_in_dim] = orig_weight

                if init_mode == "copy":
                    # Replicate pretrained weights for the new (source) channels
                    new_patch_embedding.weight[:, current_in_dim:] = orig_weight
                elif init_mode == "zeros":
                    # Initialize new channels as zeros while keeping target side intact
                    new_patch_embedding.weight[:, current_in_dim:] = 0
                elif init_mode == "random":
                    # Keep random initialization for new channels
                    pass
                else:
                ...
                # ===== 核心：权重缩放，避免通道扩张导致方差膨胀 =====
                import math
                k = new_in_dim / float(current_in_dim)  # 扩张倍数（这里=2）
                scale = math.sqrt(1.0 / k) 
                new_patch_embedding.weight.mul_(scale) #整层缩放
                new_patch_embedding.weight[:, current_in_dim:].mul_(scale) # 仅缩放新通道
                        
            # Replace the patch embedding layer
            dit_model.patch_embedding = new_patch_embedding
            dit_model.in_dim = new_in_dim
        
        # Adjust both dit and dit2 if they exist
    ...
    def __call__(
        ...
        inputs_shared = {
            ...
            "end_image": end_image,
            "input_video": input_video, "source_video": source_video, "denoising_strength": denoising_strength,
        ...
        for progress_id, timestep in enumerate(progress_bar_cmd(self.scheduler.timesteps)):
            ...
            # Inference
            noise_pred_posi = self.model_fn(**models, **inputs_shared, **inputs_posi, timestep=timestep, source_concat_mode=self.source_concat_mode)
            if cfg_scale != 1.0:
                if cfg_merge:
                    noise_pred_posi, noise_pred_nega = noise_pred_posi.chunk(2, dim=0)
                else:
                    noise_pred_nega = self.model_fn(**models, **inputs_shared, **inputs_nega, timestep=timestep, source_concat_mode=self.source_concat_mode)
...
class WanVideoUnit_InputVideoEmbedder(PipelineUnit):
    def __init__(self):
        super().__init__(
            input_params=(
                "input_video", "source_video", "noise", "tiled", "tile_size", "tile_stride", "vace_reference_image",
                "height", "width"
            ),
    ...
    def process(
        ...
        def _encode_video(video_frames):
            # 强制将输入帧尺寸对齐到调用时的 height/width，避免与噪声张量形状不一致
            if video_frames is not None:
                video_frames = [frame.resize((width, height)) for frame in video_frames]
            video_frames = pipe.preprocess_video(video_frames)
        ...
        # print(f"[WanDebug] input_video is source_video: {input_video is source_video}")
        # print(f"[WanDebug] has_target={input_video is not None}, has_source={source_video is not None}")
        target_latents = _encode_video(input_video) if input_video is not None else None
        source_latents = _encode_video(source_video) if source_video is not None else None
        # print("vae", time.time() - encode_start)
        # print(f"[WanDebug] has_target={target_latents is not None}, has_source={source_latents is not None}")
        if source_latents is None and target_latents is not None:
            source_latents = target_latents.clone()
        if target_latents is None and source_latents is not None:
            target_latents = source_latents.clone()

        if vace_reference_image is not None and (source_latents is not None or target_latents is not None):
            if not isinstance(vace_reference_image, list):
                vace_reference_image = [vace_reference_image]
            # 对参考图像也进行尺寸对齐，避免与潜变量形状不一致
            vace_reference_image = [img.resize((width, height)) for img in vace_reference_image]
            ...
            if source_latents is not None:
                source_latents = torch.concat([vace_reference_latents, source_latents], dim=2)
            if target_latents is not None:
                target_latents = torch.concat([vace_reference_latents, target_latents], dim=2)

        outputs = {}
        outputs["source_latents"] = source_latents
        if pipe.scheduler.training:
            outputs["latents"] = noise
            outputs["input_latents"] = target_latents
        else:
            outputs["latents"] = noise
            if target_latents is None:
                outputs["latents"] = noise
            else:
                latents = pipe.scheduler.add_noise(target_latents, noise, timestep=pipe.scheduler.timesteps[0])
                outputs["latents"] = latents
        # if timing_enabled and encode_start is not None:
        #     if torch.cuda.is_available():
        #         torch.cuda.synchronize()
        #     if hasattr(pipe, "_wan_debug_add_time"):
        #         pipe._wan_debug_add_time("vae", time.time() - encode_start)
        return outputs
...
class WanVideoUnit_ImageEmbedderFused(PipelineUnit):
    ...
    def process(self, pipe: WanVideoPipeline, input_image, latents, height, width, tiled, tile_size, tile_stride):
        if input_image is None or not pipe.dit.fuse_vae_embedding_in_latents:
            return {}
        # Editing with channel concat should rely on source latents instead of overwriting the first frame.wan2.2 TI2V模式下
        if pipe.source_concat_mode == "channel":
            return {}
        pipe.load_models_to_device(self.onload_model_names)
...
def model_fn_wan_video(
    ...
    source_concat_mode: str = "token",  # "token" or "channel"
    **kwargs,
):
    if sliding_window_size is not None and sliding_window_stride is not None:
        ...
        if source_latents is not None:
            tensor_names.append("source_latents")
    ...
    # Source video latent concatenation in channel dimension
    if source_latents is not None and source_concat_mode == "channel":
        ...
        x = torch.cat([x, source_latents], dim=1)  # Concatenate in channel dimension
    
    # Camera control
    x = dit.patchify(x, control_camera_latents_input)
    target_f, target_h, target_w = x.shape[2:]
    target_token_count = target_f * target_h * target_w

    # def _build_freqs(num_f, num_h, num_w, device):
    #     return torch.cat([
    #         dit.freqs[0][:num_f].view(num_f, 1, 1, -1).expand(num_f, num_h, num_w, -1),
    #         dit.freqs[1][:num_h].view(1, num_h, 1, -1).expand(num_f, num_h, num_w, -1),
    #         dit.freqs[2][:num_w].view(1, 1, num_w, -1).expand(num_f, num_h, num_w, -1)
    #     ], dim=-1).reshape(num_f * num_h * num_w, 1, -1).to(device)

    def _build_freqs(num_f, num_h, num_w, device, start_f=0, start_h=0, start_w=0):
        "Slice precomputed RoPE tables with offsets so different segments occupy unique ranges."
    ...
    # Flatten target tokens and initialize frequency segments
    target_tokens = rearrange(x, 'b c f h w -> b (f h w) c').contiguous()
    freqs_segments = [_build_freqs(target_f, target_h, target_w, target_tokens.device)]
    x_tokens = target_tokens
    # Debug info
    target_token_count_debug = target_tokens.shape[1]
    source_token_count_debug = 0

    # Source video context tokens (only in token mode)
    if source_latents is not None and source_concat_mode == "token":
        ...
        x_tokens = torch.cat([source_tokens, x_tokens], dim=1)
        freqs_segments.insert(0, _build_freqs(
            source_f, source_h, source_w, source_tokens.device,
            start_f=target_f, start_h=target_h, start_w=target_w,
        ))#0247:0 00353:target_f
        # freqs_segments.insert(0, _build_freqs(source_f, source_h, source_w, source_tokens.device))
        source_token_count_debug = source_tokens.shape[1]
    ...
    if reference_latents is not None:
        ...
        x_tokens = torch.concat([reference_tokens, x_tokens], dim=1)
        ref_f = 1
        ref_h, ref_w = reference_features.shape[2], reference_features.shape[3]
        freqs_segments.insert(0, _build_freqs(ref_f, ref_h, ref_w, x_tokens.device))

    freqs = torch.cat(freqs_segments, dim=0)
    #wan2.2 TI2V 对其t_mod和token长度
    # Align t_mod with new token length when token concat changes sequence size
    if t_mod.ndim == 4 and t_mod.shape[1] != x_tokens.shape[1]:
        token_gap = x_tokens.shape[1] - t_mod.shape[1]
        if token_gap > 0:
            # pad = torch.zeros(
            #     t_mod.shape[0], token_gap, t_mod.shape[2], t_mod.shape[3],
    ...
    if t.ndim == 3 and t.shape[1] != x_tokens.shape[1]:
        ...
        if token_gap > 0:
            # pad = torch.zeros(t.shape[0], token_gap, t.shape[2], dtype=t.dtype, device=t.device)
    ...
    #     total_tokens_debug = x_tokens.shape[1]
    #     print(f"[WanDebug] concat_mode={source_concat_mode}, source_latents={'yes' if source_latents is not None else 'no'}, "
    #           f"target_tokens={target_token_count_debug}, source_tokens={source_token_count_debug}, total_tokens={total_tokens_debug}")
    ...
    # Keep only the target tokens before reconstructing
